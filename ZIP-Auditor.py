...
class ZipAuditorApp(ctk.CTk):
    ...
    def perf_core(self, zip_path, max_length):
        ...
        try:
            ...
            # 1. Анализируем структуру архива
            with zipfile.AESZipFile(zip_path) as z_file:
                ...
                # Ищем зашифрованный файл с размером > 0 (чтобы надежно срабатывал CRC-тест ZipCrypto)
                ...
                # Если все зашифрованные файлы по 0 байт, берём первый попавшийся
                if not target_file:
                    for f_name in file_list:
                        f_info = z_file.getinfo(f_name)
                ...
                # Определяем тип шифрования через extra-поле (AES-маркер 0x9901)
                ...
                self.after(0, self.log, "log_encryption_type", enc_type, True)

            # Исправленная функция проверки пароля
            def check_password(archive_path, filename, pwd_str, is_old_crypto):
                try:
                    with zipfile.AESZipFile(archive_path) as test_z:
                        with test_z.open(filename, pwd=pwd_str.encode('utf-8')) as f:
                            if is_old_crypto:
                                # Для ZipCrypto читаем 1024 байта для проверки CRC
                                f.read(1024)
                            else:
                                # Для AES читаем 1 байт, чтобы вызвать ошибку при неверном пароле
                                f.read(1)
                    return True
                except (zipfile.BadZipFile, RuntimeError, ValueError, Exception):
                    return False

            if not self.stop_event.is_set():
                self.after(0, self.log, "log_express", True)
            
            # 2. Экспресс-анализ по базе
            for common_pass in self.BUILTIN_DATABASE:
                if self.stop_event.is_set():
                    return
                if check_password(zip_path, target_file, common_pass, is_legacy):
                    self.found_password = common_pass
                    self.stop_event.set()
                    break

            # 3. Глубокий посимвольный перебор
            if not self.found_password and not self.stop_event.is_set():
                ...
                for length in range(1, max_length + 1):
                    ...
                    for combo in itertools.product(chars, repeat=length):
                        ...
                        if check_password(zip_path, target_file, candidate, is_legacy):
