...
LOCALIZATION = {
    "Русский": {
        ...
        "btn_stop": "🛑 Прервать аудит",
        "log_init": "Лог: Оповещает каждые 100 000 комбинаций.",
        ...
        "log_mil_step": "Проверено {} тыс.",
        "log_crit_err": "❌ Критическая ошибка: {}",
        "log_found": "🎉 ПАРОЛЬ НАЙДЕН: '{}'",
        "log_stopped": "⚠️ Операция остановлена.",
    ...
    "English": {
        ...
        "log_init": "Log: Notifies every 100,000 combinations.",
        ...
        "log_mil_step": "Checked {} thousand",
        "log_crit_err": "❌ Critical error: {}",
        "log_found": "🎉 PASSWORD FOUND: '{}'",
        "log_stopped": "⚠️ Operation stopped.",
...
class ZipAuditorApp(ctk.CTk):
    # === ТОП-100 САМЫХ ПОПУЛЯРНЫХ ПАРОЛЕЙ (по статистике утечек) ===
    BUILTIN_DATABASE = [
        ...
        "sunshine", "iloveyou", "princess", "shadow", "ashley", "12345", "123456",
        "superman", "michael", "jordan", "hunter", "fuckyou", "trustno1", "batman",
        "hello", "daniel", "jessica", "soccer", "charlie", "andrew", "thomas", "joshua",
        "george", "harry", "jackson", "oliver", "william", "james", "robert", "john",
        "david", "joseph", "charles", "thomas", "christopher", "daniel", "matthew",
        ...
        "jose", "nathan", "adam", "henry", "zachary", "taylor", "emma", "olivia",
    ...
    def __init__(self):
        ...
        self.lbl_title = ctk.CTkLabel(self.header_frame, text="🛡️ ZIP-Auditor V1.0", font=("Arial", 18, "bold"))
        ...
        self.txt_log = ctk.CTkTextbox(self.tab_audit, width=510, height=180, font=("Courier New", 12))
        ...
        self.lbl_desc = ctk.CTkLabel(self.info_frame, text=self.txt["desc"], font=("Arial", 12), justify="left", anchor="nw", wraplength=500)
        self.lbl_desc.pack(padx=15, pady=(15, 5), fill="x")

        # Кликабельная ссылка на GitHub
        self.link_label = ctk.CTkLabel(
            self.info_frame,
            text=self.txt["github_link"],
    ...
    def perf_core(self, zip_path, max_length):
        ...
        try:
            with zipfile.AESZipFile(zip_path) as z_file:
                first_file = z_file.namelist()[0]

                # Экспресс (топ-100)
                self.log("log_express", force_scroll=True)
                for common_pass in self.BUILTIN_DATABASE:
                    if self.stop_event.is_set():
                        return
                    try:
                        z_file.setpassword(common_pass.encode('utf-8'))
                        z_file.read(first_file)
                ...
                if not self.found_password and not self.stop_event.is_set():
                    self.log("log_deep_start", force_scroll=True)
                    ...
                    for length in range(1, max_length + 1):
                        if self.stop_event.is_set():
                            return
                        self.log("log_len_step", length, force_scroll=True)
                        for combo in itertools.product(chars, repeat=length):
                            if self.stop_event.is_set():
                                return
                            candidate = "".join(combo)
                            tested += 1
                            # Уведомления каждые 100 000 комбинаций
                            if tested % 100000 == 0:
                                self.log("log_mil_step", tested // 1000, force_scroll=False)
                            try:
            ...
            if not self.stop_event.is_set():
                self.log("log_crit_err", str(e), force_scroll=True)
        finally:
    ...
    def brute_finished(self, elapsed):
        ...
        # Если пароль найден – показываем его независимо от stop_event
        if self.found_password:
            ...
            # Уже всё сделано в toggle_brute (очистка и сообщение об остановке)
            pass
        else:
