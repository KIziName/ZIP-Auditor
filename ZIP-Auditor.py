...
class ZipAuditorApp(ctk.CTk):
    ...
    def perf_core(self, zip_path, max_length):
        ...
        try:
            ...
            # === ОПТИМИЗИРОВАННЫЙ БЛОК ПЕРЕБОРА ===
            with zipfile.AESZipFile(zip_path) as active_z_file:

                # Исправленная функция – перехватываем ВСЕ исключения как "пароль не подошёл"
                def check_password_fast(filename, pwd_str, is_old_crypto):
                    try:
                        ...
                        # Любая ошибка при декомпрессии или дешифрации означает неверный пароль
                        return False
                ...
                for common_pass in self.BUILTIN_DATABASE:
                    if self.stop_event.is_set():
                        return
                    if check_password_fast(target_file, common_pass, is_legacy):
                    ...
                    for length in range(1, max_length + 1):
                        ...
                        for combo in itertools.product(chars, repeat=length):
                            ...
                            if tested >= next_report:
                                next_report += 250000
                                self.after(0, self.log, "log_mil_step", tested, False)
                            
                            if check_password_fast(target_file, candidate, is_legacy):
            ...
            # Если случилась критическая ошибка вне check_password_fast (повреждён архив и т.п.)
            if not self.stop_event.is_set():
